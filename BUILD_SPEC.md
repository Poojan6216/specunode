...
[2.5] DEFECTS, found by the twenty-sixth review -- six, two critical, three serious: (1, 2, critical) a replay hands a model's answer over as it arrived, and a resume after writing it on its own disk, where the run's node had it only once its journal had written it: a node racing a deadline within that write decided otherwise -- a replay sent a charge the run never made under `--dispatch`, and a resume on a quicker disk a second one. Not fixed in code: the answer's own write is not in the record, so a replay cannot wait it out; the docs, which said an answer comes back no sooner than it did, now say it comes as it arrived, sooner or later by up to a write of it, and that a deadline deciding between effects needs room past one. (3, serious) the twenty-fifth review's fix -- a replay carrying on from the recorded run's cursor -- hid any divergence, and a replay down another path reported ok: withdrawn; a replay stops again where a node's calls sit elsewhere, and a turn left running that makes it so is a documented limit. (4, serious) a turn left running, still settling as its node returned, adopted a guessed charge into the retired node and waited on its ack for ever -- the node's return is looked at before each block. (5, serious, older) LangGraph nodes run side by side lost each other's visits, a node id was minted twice and a write deduped away -- the run's cursor is merged, not overwritten. (6) a question a task left running put after its node returned was asked -- it is refused. Each code fix fails under its own planted bug. — 2026-09-26
[2.5] DEFECTS (fixed), found by the twenty-seventh review -- four, none critical, one serious, all in a turn a node left running, with guesses: (1, serious) a node that returned while such a turn was adopting a confirmed guess's charge left it neither sent nor discarded, and the turn waiting on its ack for ever -- the twenty-sixth's fix looked before the adoption, not during it; (4) a guess confirmed as its node returned was journaled after the node retired and never discarded, so a finished run read as resumable; (3) such a turn was judged by when its task got round to its answer -- behind a drafter's guess -- not when it arrived, and a replay, which guesses nothing, placed the next node elsewhere. A node's return now finishes what of its turns' guesses is being written -- a fork, a confirmation, an adoption -- then squashes the open guess and discards a confirmed one (`SpeculativeTurn.let_go`); a turn is told its answer arrived as it arrives, with how many calls are in it, and keeps a position for each. (A first try -- not guessing on a turn in a task of its own -- was dropped: on Python 3.11 `asyncio.wait_for` runs what it waits for in a task of its own.) (2) A question already being written as its node returned was refused, where the docs say it goes out: the node's return is looked at before the write only. Each fix fails under its own planted bug. — 2026-09-26
...
**1114 tests pass, 24 skip — 18 of the passes against a real Postgres 16 server.** `ruff check`,
...
**Twenty-seven independent adversarial reviews have found 238 defects here, 45 of them critical.**
The counts, in order, were **23, 17, 13, 24, 9, 6, 6, 9, 8, 7, 8, 10, 6, 7, 7, 6, 12, 7, 5, 7, 12, 8, 4, 3, 4, 6, 4.** All are fixed but three, kept and
documented: a node's writes are refused while any model turn it started is unjournaled, even one
that did not decide the write; a resume or a replay keeps the model's time but not the
machine's -- an answer comes back as it arrived from the model, where the run's node had it only
once written, so a quicker or slower disk, or a replay, hands it over sooner or later by up to a
journal write, and a call whose effect already went out returns at once: a node racing a
deadline on that edge can decide otherwise; and work a node leaves running races its return, so
a replay of a node that left a model turn running can place the next node's calls elsewhere, and
stops. That number is the most useful thing in this report, so it
is at the top rather than buried: the version of this document written a day earlier described
a finished project. The twenty-seventh to sixth reviews (2026-09-25 and -26) and the fifth (2026-09-23) are
...
### What the twenty-seventh review found

Four findings, none critical, and nothing that sends an effect twice or invents one. All four in
one seam: a turn a node left running, with guesses.

- **Serious: the twenty-sixth review's fix for a turn settling across its node's return was
  incomplete.** It looked at the node's return before adopting a confirmed guess, not during:
  a node that returned while the adoption was being written left the guessed charge neither sent
  nor discarded, and the turn waiting on its ack for ever. With it, two minor: a guess confirmed
  as its node returned was journaled after the node retired and never discarded, so a finished
  run read as resumable; and such a turn was judged by when its task got round to its answer --
  behind a drafter's guess -- not when it arrived. A node's return now finishes what of its
  turns' guesses is being written, then squashes and discards the rest, and a turn is told its
  answer arrived as it arrives.
- And one in the docs' favour: a question already being written as its node returned was
  refused, where the docs say it goes out. The node's return is looked at before the write only.

### What the twenty-sixth review found

Six findings, two critical, three serious. The two critical are one limit, now said plainly
rather than patched again; the serious three are fixed, one of them a fix withdrawn.

- **Critical, and documented rather than fixed: a replay, or a resume on a quicker disk, hands an
  answer over sooner than the run's node had it.** The run's node had each answer only once its
  journal had written it; a replay writes nothing, and a resume writes it on its own disk. A node
  racing a deadline within that write decided otherwise -- under `replay --dispatch` a charge the
  run never made went out, and a resume on a quicker disk sent a second one. The answer's own
  write is not in the record, so nothing can wait it out exactly; the docs, which said an answer
  comes back no sooner than it did, now say it comes as it arrived -- sooner or later by up to a
  write of it -- and that a deadline deciding between effects needs room past one.
- **Serious: the twenty-fifth review's fix hid divergence.** A replay carrying on from the
  recorded run's cursor after each node went on, and said ok, down paths the run never took.
  Withdrawn: a replay stops again where a node's calls sit elsewhere, and a turn left running
  that makes them sit elsewhere is a documented limit -- await every turn a node starts.
- **Serious: a turn left running could hang with a guessed charge in it.** Still settling as its
  node returned, it adopted a confirmed guess's charge into the retired node and waited on its
  ack for ever; the charge was neither sent nor discarded. The node's return is looked at before
  each block now, and the guess discarded.
- **Serious, older: LangGraph nodes run side by side lost each other's visits.** The one that
  retired last put back a cursor from before its sibling's visit; the sibling's next visit had the
  same node id and position, and its write was deduped away. The run's cursor is merged now.
- And one minor: a question a task left running put after its node returned was asked live; it
  is refused.
...
- **Serious: whether a left-over turn kept its positions still turned on timing a replay does not
  ...
  answered. The next node asked at a position the journal had no turn for. A replay was made to
  carry on after each node from where the recorded run did -- its journaled `cursor_after` --
  whatever its own timing. (Withdrawn after the twenty-sixth review: it hid real divergence too.)
...
Three findings, none critical -- the first review since the fourteenth with no critical, and it
found nothing that sends an effect twice or invents one. One serious, in the twenty-third's
repair:

- **Serious: a finished run could fail to replay.** A turn a node left running gave its
  positions back as the node returned if it was still under way -- and one whose answer came just
  before the return was still under way live, its answer being written, but done in a replay,
  which writes nothing. The next node asked at a position the journal had no turn for. A
  left-over turn is judged now by whether the model's whole answer had arrived, which live, a
  resume and a replay pace the same.
- And two minor: adapters.md said what a node left running reaches the world no further, where
  a read already sent and a question already asked finish -- it says so now; and the
  twenty-third review changed where a left-over turn leaves the cursor without changing the
  position rule's number -- it is 4.

### What the twenty-third review found

Four findings, one critical -- in the twenty-second review's repair of where a turn ends.

- **Critical: a client closing its connection could still decide for the node.** A turn ended at
  its answer, but its stream then waited for the client to close -- and a client that closes in
  its own `async with` exit, as one built on an HTTP stream context does, was waited for on the
  node's time, uncancellably: a deadline that fired meanwhile was swallowed. The run priced an
  order at 2.6 s; its resume, with no connection to close, fell back at 1 s and charged a
  second, different amount, and said ok. The client now closes its stream in the background.
- **Serious: a turn left running could still start a guess after its node returned.** Checked
  once, before the drafter was asked and the fork written, a guess opened after the return -- a
  drafter slow to guess, a fork being written as the node returned -- and its call reached the
  upstream, never resolved or resolved before its fork. Looked at again after each wait; the
  fork goes through the drive guard; and the squash as the node returns waits for a fork being
  written.
- And two minor: a turn left running kept the positions its blocks took while the node ran if it
  failed after the return, and gave them back if it failed before -- it gives them back as the
  node returns; and a write left running that began before the return was staged after it -- it
  is refused where it would be staged.

### What the twenty-second review found

Eight findings, four critical -- two in the twenty-first review's repairs, two older.

- **Critical: a run recorded under another position rule could be resumed under this one.** The
  check judged each failed turn by what the journal said of it: a turn `call_turn` streamed from
  a default envelope was journaled as a request for no stream, and not counted; and the
  twenty-first review changed where calls sit after a turn a deadline cut short without changing
  the rule's number. Both resumed an earlier version's run and charged twice. The rule is 3 now,
  and a run any part of which was recorded under another rule is refused whole -- resume, replay
  and `status` alike.
- **Critical, older: an answer on disk could be given up on.** A turn was journaled as answered
  and its stream read on while the client closed its connection; a deadline fired meanwhile, the
  node charged a fallback, and a resume -- served a stream that ends at its answer -- took the
  model's path as well. A client that failed as it closed had its error handed over in place of
  the answer. A turn is over now once its answer is on disk and handed over.
- **Critical, older, and not fixed in code: a resume on a slower machine can decide otherwise.**
  A served answer is handed over only after this process writes the question and the answer
  again, and a call whose effect already went out returns at once. On a disk slower than the
  run's the answer can come late -- the question's write is hidden in the time the model took,
  the answer's is not -- and a node racing a deadline on that edge charged a fallback the run
  never made. `docs/replay.md` said the pace held
  however slow the disk; it now says how far it holds: the model's time, not the machine's.
- **Serious: what a node left running still moved its positions as it retired.** A left-over
  turn's block, the failure of one, or a left-over call arriving while the node's retirement was
  being written moved the position that retirement journaled; the replay, on a quick disk, did
  not, and a finished run did not replay. Nothing a node left running takes a position once its
  body returns, and its calls are refused.
- **Serious: a left-over turn's guess was settled after the run.** Squashed or confirmed after
  `run_finished`, and a confirmed one made the finished run read as resumable. The open guesses
  of turns a node left running are squashed as it returns, and a turn's bookkeeping writes
  nothing once its drive is over.
- And two minor: the rule check refused a run whose failed turn's node had retired in a later
  attempt -- moot once any other rule refuses; and mypy, held to Python 3.11 under 3.12, could
  not read numpy's stubs -- it checks as the interpreter it runs under.
